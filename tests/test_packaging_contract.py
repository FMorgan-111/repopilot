...
_EVAL_LOCK_HEADER = (
    ...
    "#    uv pip compile requirements-eval.in "
    "--python-platform x86_64-unknown-linux-gnu --python-version 3.11 "
    "--generate-hashes --output-file requirements-eval.lock",
)
...
def _assert_eval_lock_provenance(text: str) -> None:
    assert tuple(text.splitlines()[:2]) == _EVAL_LOCK_HEADER
...
def test_eval_lock_binds_the_approved_uv_target_and_command() -> None:
    _assert_eval_lock_provenance(
        (ROOT / "requirements-eval.lock").read_text(encoding="utf-8")
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (" --generate-hashes", ""),
        ("compile requirements-eval.in", "compile requirements.txt"),
        (
            "--output-file requirements-eval.lock",
            "--output-file requirements.txt",
        ),
    ],
)
def test_eval_lock_provenance_rejects_command_mutations(
    approved: str,
    replacement: str,
) -> None:
    text = (ROOT / "requirements-eval.lock").read_text(encoding="utf-8")
    mutated = text.replace(approved, replacement, 1)
    assert mutated != text

    with pytest.raises(AssertionError):
        _assert_eval_lock_provenance(mutated)
