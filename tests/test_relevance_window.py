...
hallucinate search blocks. `relevance_window` centers the same char budget on
the issue-relevant lines instead.
"""

from src.escalation import relevance_window


def test_short_content_returned_whole():
    content = "line1\nline2\n"
    assert relevance_window(content, ["anything"], 6000) == content
...
def test_centers_on_matching_line_below_head():
    ...
    body = "def target_fn():\n    return compute_bug()"
    content = head + "\n" + body
    window = relevance_window(content, ["target_fn"], 400)
...
def test_no_term_match_falls_back_to_head():
    ...
    window = relevance_window(content, ["nonexistentxyz"], 200)
...
def test_window_respects_char_budget_roughly():
    ...
    limit = 300
    window = relevance_window(content, ["line250"], limit)
...
def test_picks_densest_term_line():
    ...
    window = relevance_window(content, ["foo", "bar", "baz", "qux"], 300)
