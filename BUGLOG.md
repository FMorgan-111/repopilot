...
**现象**：`_extract_json()` 的备用正则 `\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}` 最多处理一层嵌套的 `{}`。遇到 `{"a":{"b":{"c":1}}}` 这种两层以上的会解析失败。

**根因**：正则表达式无法表达任意深度的递归匹配。

**影响**：实际很低。99% 的情况 LLM 返回的原始文本先走 `json.loads()` 直接解析成功。这个正则是兜底，只有 LLM 把 JSON 嵌在散文里且有三层以上嵌套时才触发。
...
for i in range(start, len(text)):
    if text[i] == '{': depth += 1
        ...
        depth -= 1
        if depth == 0:          # 找到匹配的最外层右括号
            return json.loads(text[start:i+1])
```

遇到 `{` 计数器+1，遇到 `}` 计数器-1，回到 0 时就是正确的最外层 JSON 结束位置——任意深度嵌套都能切出来。
